...
if __name__ == '__main__':
    ...
    (X_train, y_train), (X_val, y_val), (X_test, y_test) = get_data(args.input)

    dataset_train = Dataset(X_train, y_train, 5000, 'gpu:0', shuffle=True)
    dataset_val = Dataset(X_val, y_val, 5000, 'gpu:0')
    ...
    beta_sched = BetaScheduler(PieceWiseSchedule([(0, 1e-6, 'constant'), (4000, 1e-6, 'log'), (200000, 5e-4, 'constant')]))
    lr_sched = LearningRateScheduler(
        cosine_decay_restarts_schedule(1e-3, 4000, t_mul=1.0, m_mul=0.91, alpha=1e-6, alpha_steps=200)
    )
